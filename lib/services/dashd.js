...
Dash.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  /*
  so, just change it to calculate the hash of the hex buffer by converting it
  from hex to binary and then hashing it...or just use tx.hash
  and convert it to the format that zmqKnownTransactionLocks expects”
  */

  function addTxlockToTx(tx, result) {
    tx.txlock = false;
    if (result.instantlock || result.chainlock) {
      tx.txlock = true;
      return;
    }
    var id = tx.hash.toString('binary');
